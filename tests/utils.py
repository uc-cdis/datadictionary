...
specific overrides are not being used in the GDC currently.
...
DATA_DIR = os.path.join(os.path.dirname(SCHEMA_DIR), 'examples')
project1 = load_yaml(os.path.join(SCHEMA_DIR, 'projects/project1.yaml'))
...
class BaseTest(unittest.TestCase):
    ...
    def setUp(self):
        self.dictionary = GDCDictionary(root_dir=SCHEMA_DIR)
        self.definitions = load_yaml(os.path.join(SCHEMA_DIR, '_definitions.yaml'))
...
def check_for_cycles(schemata, ignored_types=None):
    ...
    for schema_type, schema in schemata.items():
        ...
        for link in schema.get('links', []):
            if 'subgroup' in link:
                target_types = [g.get('target_type') for g in link['subgroup']]
            else:
