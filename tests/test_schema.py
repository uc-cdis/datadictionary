...
from tests.utils import BaseTest, check_for_cycles, validate_schemata
...
def _check_enum(enums):
    ...
    for path, enum_list in enums.items():
        ...
        for item in enum_list:
            # dictionaryutils >= 3.1.0 adds None to non-required enums.
            if not isinstance(item, str) and item is not None:
                check_result["enum item is not a string or None"].append(f"{path}: {item}")
...
class SchemaTest(BaseTest):
    ...
    def test_acyclicality(self):
        ...
        # Set ignored to empty list since 'archive' and 'file' are not included in schemas
